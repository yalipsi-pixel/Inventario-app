...
# Función auxiliar para manejar entrada manual o mediante botón de cámara
...
    st.markdown(
        "_Escribe el código o presiona **📸 Cámara** para abrir la cámara de tu celular. Si el producto existe, se completarán sus datos automáticamente._"
    )
...
            prov_sug = match.iloc[0]["Proveedor"]
...
    
    bodegas_disponibles = ["-- Seleccionar --", "Huingan", "Bucalemu", "Rinconada", "San Felipe"]
    bodega = st.selectbox("Bodega", bodegas_disponibles, index=0)
    
    proveedores_disponibles = ["-- Seleccionar --", "Copeval", "Gmt", "M&Valdivieso", "Otro"]
    proveedor = st.selectbox("Proveedor", proveedores_disponibles, index=0)
...
    st.markdown(
        "_Selecciona el producto del inventario, usa la cámara o ingresa el código._"
    )
...
    
    campos_disponibles = ["-- Seleccionar --", "El Huingan", "Bucalemu", "Rinconada", "San Felipe"]
    campo = st.selectbox("Campo", campos_disponibles, index=0)
    
    cuarteles_disponibles = ["-- Seleccionar --", "Cuartel 1 Timpson", "Cuartel 2", "Cuartel 3", "General / Bodega"]
    cuartel = st.selectbox("Cuartel", cuarteles_disponibles, index=0)
    
    usuarios_disponibles = ["-- Seleccionar --", "Bruno Hernández", "Manuel Muñoz", "Bodeguero"]
    usuario = st.selectbox("Usuario / Responsable", usuarios_disponibles, index=0)
...
        else:
            st.error("Debe ingresar o seleccionar un producto válido y una cantidad mayor a 0.")

# --- VISTA 3: INVENTARIO GENERAL ---
else:
